    ...
    for row in csv_file:
        date_of_transaction = \
            datetime.datetime.strptime(row['\xc3\xaf\xc2\xbb\xc2\xbf Date'
                ], '%d/%m/%Y')  # converts the date of the current row into a readable Python format
        ...
        if row['Type'] in c_or_d:  # checks if the 'type' of the transaction is in the tuple 'c_or_d' argument
            ...
            if date_of_transaction.year in credit_or_debit_dictionary:  # checks if the year of the credit is in the dictionary
                ...
                if date_of_transaction.month \
                    in credit_or_debit_dictionary[date_of_transaction.year]:  # checks if the month is in the dictionary under that year
                    current_amount = \
                        credit_or_debit_dictionary.get(date_of_transaction.year).get(date_of_transaction.month)  # gets the current value for that month
                    credit_or_debit_dictionary[date_of_transaction.year][date_of_transaction.month] = \
                        current_amount + float(transaction_amount[3:])  # updates the value with the previous value + the credit value of the current row
    ...
    for amount in c_or_d_lst:
        if counter == c_or_d_lst.index(amount):  # checks if we are on the first iteration and if so continues to the next without doing anything
            continue
              ...
              # will execute provided we have two different months to compare
